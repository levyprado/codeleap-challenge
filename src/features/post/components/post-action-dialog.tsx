...
const PostActionDialogContext = createContext<{ close: () => void } | null>(
  null,
)

export const usePostActionDialog = () => {
  const context = use(PostActionDialogContext)
  if (!context)
    throw new Error('usePostActionDialog must be used within PostActionDialog')
  return context
}
...
export default function PostActionDialog({
  ...
  return (
    <PostActionDialogContext value={{ close }}>
